...
function getFirstIpAddress(cidrStr, callback) {
  ...
  if (!cidr.isValid()) {
    ...
    // Saves the returned ipv4 value to the object to be returned.
    ipv4_ipv6_add["ipv4"] = firstIpAddress;
    // Generates and saves the ipv6 value from the ipv4 value to the object to be returned.
    ipv4_ipv6_add["ipv6"] = getIpv4MappedIpv6Address(firstIpAddress);
  }
  // Call the passed callback function.
  // Node.js convention is to pass error data as the first argument to a callback.
  // The IAP convention is to pass returned data as the first argument and error
  // data as the second argument to the callback function.
  return callback(ipv4_ipv6_add, callbackError);
}
